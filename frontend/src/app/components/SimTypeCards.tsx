      ...
      {simTypes.map((sim) => {
        ...
        return (
          <Link
            key={sim.href}
            href={sim.href}
            className={`card group p-4 transition-all ${
              isActive ? 'border-gold/50 bg-gold/[0.03]' : 'hover:border-gold/20'
            }`}
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="hidden text-[11px] text-muted sm:block">{sim.description}</p>
              </div>
            </div>
          </Link>
        );
